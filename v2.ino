...
void candyStripes(uint8_t width, CRGB color1, CRGB color2, uint8_t speed ) {
    for (int i =  NUM_LEDS - 1; i > 0; i--) {
      leds[ledArray[i]] = leds[ledArray[i - 1]];
    }

    //reset
    EVERY_N_MILLIS_I( Dot_time, speed) {
      Dot_time.setPeriod( 500 );
      leds_done = 0;
    }

    if (leds_done < width) {
      leds[ledArray[0]] = color1;
      leds_done = leds_done + 1;
    } else {
      leds[ledArray[0]] = color2;
